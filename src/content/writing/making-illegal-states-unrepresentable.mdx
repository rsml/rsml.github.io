...
In the example above, `Shape` is the part that's specifically called a "discriminated union." The word "descriminated" refers to the fact that they all share a property that distinguishes them, which in this example is called "kind."
...
[Effect](https://effect.website/) comes to the rescue as a very opinionated way to make both errors and requirements (the dependencies a function needs) an explicit part of its type.
